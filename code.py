...
def group_collide(group, other_object):
    for el in list(group):
        if el.collide(other_object):
            explosion = Sprite(el.get_position(), [0, 0], 0, 0, explosion_image, explosion_info, explosion_sound)
...
class Sprite:
    ...
    def draw(self, canvas):
        if not self.animated:
            ...
            global exp
            explosion_dim = 24
            explosion_index = (exp % explosion_dim) // 1
            current_explosion_center = [self.image_center[0] +  explosion_index * self.image_size[0], self.image_center[1]]
...
def draw(canvas):
    global time, started, lives, score, num_of_rocks, num_of_collisions
    ...
    # Collisions between ship, missiles, and rocks
    ...
    # draw splash screen if not started or if player lose
    if not started:
...
def rock_spawner():
    global a_rock, rock_group, num_of_rocks, started
    rock_pos = [random.randrange(0, WIDTH), random.randrange(0, HEIGHT)]
    rock_vel = [random.random() *0.6 + score - .3, random.random() * 0.6 + score - .2]
    rock_avel = random.random() * .2 - .1
    a_rock = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)
    if num_of_rocks<12 and started and (not a_rock.collide(my_ship)):
